...
type Item = {
  type: "checkup" | "chronic_control" | "kinesiology" | "prescription_renewal" | "weight_management"; requestId: string; orderId: string | null;
...
type Result = { items: Item[]; counts: { total: number; checkup: number; chronicControl: number; newServices: number; last24Hours: number; emailPending: number } };
const typeLabels: Record<Item["type"], string> = { checkup:"Chequeo", chronic_control:"Control crónico", kinesiology:"Kinesioterapia", prescription_renewal:"Renovación de receta", weight_management:"Control de peso" };
...
export default function AutomaticOrdersClient() {
  ...
  return <div className="space-y-7">
    <PageHeader eyebrow="Validación" title="Revisar validación automática" description="Órdenes aprobadas por protocolos clínicos versionados. Incluye chequeos, controles crónicos y los nuevos servicios, sin requerir aprobación médica manual para su emisión." aside={<button type="button" onClick={() => void load()} disabled={loading} className="rounded-xl border border-emerald-300 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-800 hover:bg-emerald-50 disabled:opacity-60">Actualizar</button>} />
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <Metric label="Nuevos servicios" value={result?.counts.newServices ?? 0} />
