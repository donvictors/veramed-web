...
export default function ChronicControlSummaryPage() {
  ...
  useEffect(() => {
    ...
    void fetchChronicControlRequest(requestId)
      .then((request) => {
        const storedMethod = request.rec.tests.some((test) => test.name === "Colonoscopía total")
          ? "colonoscopy"
          : "fit";
        const storedCervicalMethod = request.rec.tests.some(
          (test) =>
            test.name === "Cotesting (PAP+VPH)" ||
            (request.rec.tests.some((item) => item.name === "Papanicolau (PAP)") &&
              request.rec.tests.some((item) => item.name === "PCR de virus papiloma humano (VPH)")),
        )
          ? "cotesting"
          : request.rec.tests.some(
                (test) =>
            ...
            ? "hpv"
            : "pap";
        const storedBloodPressureMethod = request.rec.tests.some(
          (test) => test.name === "Holter de presión arterial (MAPA)",
        )
          ? "mapa"
          : "skip";
        const storedBreastImaging = request.rec.tests.some(
          (test) => test.name === "Ecografía mamaria",
        )
          ? "mammo_plus_ultrasound"
          : "mammo_only";
        const storedProstateMethod = request.rec.tests.some(
  ...
  const hasColorectalScreening =
    knownNames.has("Tamizaje de cáncer colorrectal") ||
    knownNames.has("Test inmunológico de sangre oculta en deposiciones") ||
    knownNames.has("Colonoscopía total");
  const hasCervicalScreening =
    knownNames.has("Tamizaje de cáncer cervicouterino") ||
    knownNames.has("Papanicolau") ||
    knownNames.has("Papanicolau (PAP)") ||
    knownNames.has("Test de VPH (HPV)") ||
    knownNames.has("PCR de virus papiloma humano (VPH)") ||
    knownNames.has("Cotesting (PAP+VPH)");
  const hasBloodPressureScreening = knownNames.has("Holter de presión arterial (MAPA)");
  const hasBreastScreening =
    knownNames.has("Tamizaje de cáncer de mama") ||
    knownNames.has("Mamografía bilateral") ||
    knownNames.has("Ecografía mamaria");
  const hasProstateScreening = knownNames.has("Antígeno prostático específico (APE)");
  ...
  const displayedTests = data.rec.tests
    .flatMap((test) => {
  ...
  const summaryCounts = displayedTests.reduce(
    (acc, test) => {
      ...
      else if (category === "interconsultation") acc.interconsultation += 1;
      else acc.laboratory += 1;

      return acc;
    },
    { laboratory: 0, image: 0, procedure: 0, interconsultation: 0 },
  );
  const orderDetails = inferOrderDetails(displayedTests);
  ...
  const selectedOptionalAdditionalTests = optionalAdditionalTests.filter((test) =>
    displayedTests.some((item) => item.name === test.name),
  );
  const optionalAdditionalTestsToShow = optionalAdditionalTests.filter(
    (test) => !displayedTests.some((item) => item.name === test.name),
  );

  async function handlePreferenceChange(
    preferences: Parameters<typeof updateChronicControlScreeningPreferences>[1],
  ) {
    if (!requestId) {
      return;
    }
    ...
    try {
      const updated = await updateChronicControlScreeningPreferences(requestId, preferences);
      startTransition(() => {
        setData(updated);
      });
    } catch (error) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 text-base leading-7 text-slate-600">
              Se identificaron {displayedTests.length} exámenes para tus necesidades de control.
            </p>
          </div>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_22px_70px_-48px_rgba(15,23,42,0.45)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-slate-300 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">
                ${getChronicControlTotalPrice(data.rec).toLocaleString("es-CL")}
              </span>
            </div>

            {hasGeneralCheckup && hasCustomizableScreenings && (
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      {bloodPressureInfoOpen && (
                        <p className="mt-2 text-sm leading-6 text-slate-600">
                          Las guías internacionales recomiendan el tamizaje de presión arterial una
                          vez al año en mayores de 18 años, pero no recomiendan un método
                          específico. Creemos que el Holter de presión arterial es el método más
                          práctico para personas que desean chequear esto en su casa, sin embargo no
                          es el único. Una alternativa es la medición en la consulta por un
                          profesional de salud, por lo que si en el último año te han medido la
                          presión y resultó normal (&lt;140/90), puedes no hacerte este examen. Otra
                          alternativa es medirte tu la presión en la casa, lo que se conoce como
                          automonitoreo de presión arterial (AMPA), pero esto es importante hacerlo
                          con aparatos de presión que la midan en el brazo (¡no en la muñeca!) y
                          que estén validados. Si quieres saber si un equipo está validado, puedes
                          revisarlo{" "}
                          <a
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          checked={checked}
                          onChange={(event) =>
                            void handleOptionalAdditionalTestChange(test.name, event.target.checked)
                          }
                          className="mt-1"
                        />
                        <div className="flex min-w-0 items-start gap-3">
              ...
              {displayedTests.map((test) => (
                <div key={test.name} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() => void handleRemoveTest(test.name)}
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-white p-6">
              ...
              <div className="mt-6 grid gap-4">
                ...
                <MetricCard
                  ...
                  value={`Set recomendado: ${summaryCounts.laboratory} examen(es) de laboratorio, ${summaryCounts.image} examen(es) de imagen, ${summaryCounts.procedure} procedimiento(s), ${summaryCounts.interconsultation} interconsulta(s)`}
                />
                <MetricCard
...
function renderClinicalNote(note: string): ReactNode {
  if (
    note ===
    ...
    return (
      <>
        Te sugerimos complementar estos exámenes con el autocontrol de tu presión arterial.
        Recuerda usar un{" "}
        <a
          href="/documentos/ampa-validados.pdf"
          target="_blank"
        ...
        para guiarte en como hacer el registro.
      </>
    );
  }
