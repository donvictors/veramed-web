...
export default function CheckupPaymentValidationPage() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("veramed_payment_pending");
    if (!raw) {
    ...
    const confirmPayment = setTimeout(() => {
      ...
      localStorage.setItem("veramed_payment", JSON.stringify(confirmedPayment));
      localStorage.removeItem("veramed_payment_pending");
