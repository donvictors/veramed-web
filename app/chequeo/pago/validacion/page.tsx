...
import { confirmCheckupPayment, fetchCheckupRequest } from "@/lib/checkup-api";

export default function CheckupPaymentValidationPage() {
  ...
  useEffect(() => {
    ...
    const confirmPayment = setTimeout(() => {
      void fetchCheckupRequest(requestId)
        .then(() => confirmCheckupPayment(requestId))
        .then(() => {
          router.push(`/chequeo/estado?id=${requestId}`);
        })
        .catch(() => {
          router.replace(`/chequeo/pago?id=${requestId}`);
        });
    }, 3000);
      ...
      <div className="mx-auto max-w-4xl px-6 py-10 md:py-12">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-base leading-7 text-slate-600">
            Una vez validado, te llevaremos automáticamente al estado de emisión de tu orden.
          </p>
        </div>
